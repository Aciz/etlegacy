...
static qboolean point_in_brush(const vec3_t point, cbrush_t *brush);
...
static float *get_uv_coords(vec2_t uv, const vec3_t vert, vec3_t normal);
...
static vec4_t clipweap_color       = { 96, 0, 96, 255 };
static vec4_t clipweap_metal_color = { 37, 58, 61, 255 };
static vec4_t clipweap_wood_color  = { 33, 33, 0, 255 };
...
void tc_vis_init(void)
{
	...
	triggers_draw = Cvar_Get("r_drawTriggers", "0", CVAR_ARCHIVE | CVAR_CHEAT);
	clips_draw    = Cvar_Get("r_drawClips", "0", CVAR_ARCHIVE | CVAR_CHEAT);
	slicks_draw   = Cvar_Get("r_drawSlicks", "0", CVAR_ARCHIVE | CVAR_CHEAT);

	trigger_shader_setting = Cvar_Get("r_drawTriggersShader", "tcRenderShader", CVAR_ARCHIVE | CVAR_LATCH);
	clip_shader_setting    = Cvar_Get("r_drawClipsShader", "tcRenderShader", CVAR_ARCHIVE | CVAR_LATCH);
	slick_shader_setting   = Cvar_Get("r_drawSlicksShader", "tcRenderShader", CVAR_ARCHIVE | CVAR_LATCH);
...
static void add_triggers(void)
{
	...
	while (1)
	{
		...
		char *token = COM_Parse((char **) &entities);
		if (!entities)
		{
			break;
		}
		...
		while (1)
		{
			...
			{
				token      = COM_Parse((char **) &entities);
				is_trigger = Q_stristr(token, "trigger") != NULL;
			}
...
static inline qboolean angleSlick(cplane_t const *plane)
{
...
static void draw(visBrushNode_t *brush, qhandle_t shader)
{
	...
	while (brush)
	{
		for (i = 0; i < brush->numFaces; ++i)
		{
			...
			re.AddPolysToScene(shader, brush->faces[i].numVerts, brush->faces[i].verts, 1);
		}
		brush = brush->next;
	}
}
