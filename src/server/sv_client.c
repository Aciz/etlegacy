...
gotnewcl:
	...
	*newcl    = temp;
	...
	denied = (char *)(VM_Call(gvm, 5, GAME_CLIENT_CONNECT, clientNum, qtrue, qfalse)); // firstTime = qtrue
	if (denied)
	{
...
void SV_DropClient(client_t *drop, const char *reason)
{
	...
	VM_Call(gvm, 5, GAME_CLIENT_DISCONNECT, drop - svs.clients);
...
void SV_ClientEnterWorld(client_t *client, usercmd_t *cmd)
{
	...
	VM_Call(gvm, 5, GAME_CLIENT_BEGIN, client - svs.clients);
}
...
void SV_UpdateUserinfo_f(client_t *cl)
{
	...
	VM_Call(gvm, 5, GAME_CLIENT_USERINFO_CHANGED, cl - svs.clients);
}
...
void SV_ExecuteClientCommand(client_t *cl, const char *s, qboolean clientOK, qboolean premaprestart)
{
	...
	if (clientOK)
	{
		...
		if (!u->name && sv.state == SS_GAME && (cl->state == CS_ACTIVE || cl->state == CS_PRIMED || cl->demoClient))
		{
			...
			Cmd_Args_Sanitize();
			VM_Call(gvm, 5, GAME_CLIENT_COMMAND, cl - svs.clients);
		}
	}
	else if (!bProcessed)
	{
...
void SV_ClientThink(client_t *cl, usercmd_t *cmd)
{
	...
	VM_Call(gvm, 5, GAME_CLIENT_THINK, cl - svs.clients);
}
